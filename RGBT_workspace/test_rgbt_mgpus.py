...
def genConfig(seq_path, set_type):
    ...
    elif set_type == 'DRGBT603':
        RGB_img_list = sorted([seq_path + '/up/' + p for p in os.listdir(seq_path + '/up') if p.endswith(".jpg")])
        T_img_list = sorted([seq_path + '/down/' + p for p in os.listdir(seq_path + '/down') if p.endswith(".jpg")])

        RGB_gt = np.loadtxt(seq_path + '/init.txt', delimiter=',')
        T_gt = np.loadtxt(seq_path + '/init.txt', delimiter=',')
        # target_gt = np.loadtxt(seq_path + '/exist.txt', delimiter=',')
        with open(seq_path + '/exist.txt','r') as f:
            target_gt = [line.strip() for line in f] 
        return RGB_img_list, T_img_list, RGB_gt, T_gt,target_gt
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='adapter'):
    ...
    if script_name == 'bat' or script_name == 'drgbt':
    ...
    RGB_img_list, T_img_list, RGB_gt, T_gt,Target_gt = genConfig(seq_path, dataset_name)
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            # initialization
            image = get_x_frame(rgb_path, T_path)
            tracker.initialize(image, RGB_gt[0].tolist())  # xywh
        elif frame_idx > 0:
            if Target_gt[frame_idx] == '1':
                # track
                image = get_x_frame(rgb_path, T_path)
                region, confidence = tracker.track(image,is_changed=True)  # xywh
                result[frame_idx] = np.array(region)
            elif Target_gt[frame_idx] == '0':
                # track
                image = get_x_frame(rgb_path, T_path)
                region, confidence = tracker.track(image,is_changed=False)  # xywh
                result[frame_idx] = np.array(region)
...
class BAT_RGBT(object):
    ...
    def track(self, img_RGB,is_changed):
        '''TRACK'''
        outputs = self.tracker.track(img_RGB,is_changed)
...
if __name__ == '__main__':
    ...
    if dataset_name == 'GTOT':
        ...
        seq_home = '/data1/Code/dingzhaodong/BAT/data/LasHeR'
    ...
    elif dataset_name == 'DRGBT603':
        seq_home = '/data/wuyingjie/datasets'
        with open('evaluate_DRGBT603/test_set.txt','r') as f:
