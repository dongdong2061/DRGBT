...
from ..layers.attn_adapt_blocks import CEABlock,CEABlock_Dynamic   ##BAT 
...
from ..layers.modules import SiameseTracker
...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.template_modal_consistent = False
        self.search_modal_consistent = False

        
        self.relocalizator = SiameseTracker()


    def forward_features(self, z, x, mask_z=None, mask_x=None,
                         ce_template_mask=None, ce_keep_rate=None,
                         return_last_attn=False,Test=None,localization=None,second_phase=None):

        self.template_modal_consistent = False
        self.search_modal_consistent = False

        #for updated templates, not use
        zB,_,_,_ = z.shape
        if zB == 2:
            ...
            z = z_init_ = self.patch_embed(z)
            zi = zi_init_ = self.patch_embed(zi)
            zi_update_ = self.patch_embed(zi_update)
            z_update_ = self.patch_embed(z_update)
        else:
            z_init = z[:, :3, :, :]
            zi_init = z[:, 3:, :, :]
            B, H, W = x.shape[0], x.shape[2], x.shape[3]
        ...
        if second_phase:
            xcorr,loss = self.relocalizator(z_init,zi_init,x_rgb,x_dte,Test)
            # corr_i = self.relocalizator(zi,xi)
            return xcorr,loss
...
###################################################################===========
        ...
        if zB == 2:
            z_update_ += self.pos_embed_z
            zi_update_ += self.pos_embed_z
            x = torch.cat([z_update_,x],dim=1)
            xi = torch.cat([zi_update_,xi],dim=1)
        ...
        for i, blk in enumerate(self.blocks):
            ...
            # if i == 0 :
                ...
                xi, global_index_ti, global_index_si, removed_index_si, attn_i,attn_loss = \
                    blk(x, xi, global_index_t, global_index_ti, global_index_s, global_index_si, mask_x, ce_template_mask,
                        ce_keep_rate,Test,search_modal_consistent=self.search_modal_consistent)
                loss.append(attn_loss)
            else:       
                x, global_index_t, global_index_s, removed_index_s, attn, \
        ...
        if Test:
            if self.search_modal_consistent:
                x_up,x_down = x,xi
                x = (x_up,x_down)
            else:
                x = x + xi
                # x_ = self.adap_fusion(x_)
        else:
            x = x + xi
    ...
    def forward(self, z, x,img_blocks=None,ts_images=None, ce_template_mask=None, ce_keep_rate=None,
                tnc_keep_rate=None,
                return_last_attn=False,Test=None,localization=None,second_phase=None):
        if second_phase:
            loss = self.forward_features(z, x,img_blocks,ts_images,ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate,Test=Test,localization=localization,second_phase=second_phase)
            return loss
        else:
            x, aux_dict = self.forward_features(z, x,img_blocks,ts_images,ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate,Test=Test,localization=localization,second_phase=second_phase)
            return x, aux_dict
